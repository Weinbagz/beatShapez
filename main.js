    ...
    constructor() {
        ...
        let A = function (A) {
            ...
            return function () {
                ...
                return (Q = (Q + 1) | 0), (g = B ^ (B >>> 9)), (B = (D + (D << 3)) | 0), (D = ((D = (D << 21) | (D >>> 11)) + A) | 0), (A >>> 0) / 4294967296;
            };
        };
        (this.prngA = new A(tokenData.hash.substring(2, 34))), (this.prngB = new A(tokenData.hash.substring(34, 66)));
...
loadingOverlay.style.zIndex = "999"; // Set a high z-index value to ensure it appears on top
document.body.appendChild(loadingOverlay);

let loadingText = document.createElement("div");
loadingText.innerText = "Loading...";
loadingText.style.color = "#02e1e8"; // Change the text color here
loadingText.style.fontSize = "7vw"; // Change the font size here
loadingText.style.fontFamily = "IBM Plex Mono, sans-serif"; // Change the font family here
loadingText.style.position = "absolute"; // Set the position to absolute
loadingText.style.top = "10px"; // Adjust the top position here (in pixels or any other unit)
loadingText.style.left = "10px"; // Adjust the left position here (in pixels or any other unit)
...
// If the external assets are available, use them. If not, use the hardcoded CIDs 
const externalOneCID = tokenData.externalAssetDependencies?.[0]?.cid || 'VXHjZWr9S5zeJxVTYB6KBchdl1YWO9rayMPQVajCVts';
const externalTwoCID = tokenData.externalAssetDependencies?.[1]?.cid || 'GH0xkOLcvY1nFHRP572Avl06LIwjlEN1jQk8_2kqwug';
const externalThreeCID = tokenData.externalAssetDependencies?.[2]?.cid || 'I4GEsae3uX3uIHNPA9X9j0RJ78rf1PeWRtt4OJkdW0o';
const externalFourCID = tokenData.externalAssetDependencies?.[3]?.cid || 'ewLFa6BC_Xrsc-ed-p8qQlQaqy7ehQjqw4fZzrowNP0';
const externalFiveCID = tokenData.externalAssetDependencies?.[4]?.cid || '_rHhufGaspOFK5SxCdbLMpcoEVScEaUv0R0OpkROuUc';
const externalSixCID = tokenData.externalAssetDependencies?.[5]?.cid || 'zNTU8zpvTo-0p2XYsqdbaUR-3zc3De0fo8MnubN0NII';
const externalSevenCID = tokenData.externalAssetDependencies?.[6]?.cid || 'bTHoujJe9z0PMS8fI7xLpwaMRQvFnhj0blRf25-BX3E';
const externalEightCID = tokenData.externalAssetDependencies?.[7]?.cid || 't1Jq53QzozwmpI5pSyA3pRaT_S-jai1usiEynGeQc98';
const externalNineCID = tokenData.externalAssetDependencies?.[8]?.cid || 'NsX0HDBzr0xRWwdYh5g3Clr9ikSHiVTxGiye4zmlc74';
const externalTenCID = tokenData.externalAssetDependencies?.[9]?.cid || 'CA0IWhuhsFc8HVkMX8eW7Om5-j5uHFRWRF6Xu9WA9Fc';
const externalElevenCID = tokenData.externalAssetDependencies?.[10]?.cid || 'aWytmvdk5do0AHb6qXPmw5Ahx3jlHYqa5TafDODfwA8';
const externalTwelveCID = tokenData.externalAssetDependencies?.[11]?.cid || '-nPYH5i-eJWKnbAgY25LBYZct6vl8WluxRB_eR4Md_c';
const externalThirteenCID = tokenData.externalAssetDependencies?.[12]?.cid || 'VCyclSlIcQqHLuG5jhxfTjAb_O-9pXydMP0D7SdZ4dE';
const externalFourteenCID = tokenData.externalAssetDependencies?.[13]?.cid || 'xtdPdk6ExhbLQVBR39mwnRLc6b4x72MmOTL4hs-0MiA';
const externalFifteenCID = tokenData.externalAssetDependencies?.[14]?.cid || 'hmtnEa4gmyS2XmUTDyoS1Fq0GN2FTv7nWF_09q9ry40';
const externalSixteenCID = tokenData.externalAssetDependencies?.[15]?.cid || 'khW0_54FzCGdQeROyNZf9eXo2v2GPDju1H2rEG3zRYg';
const externalSeventeenCID = tokenData.externalAssetDependencies?.[16]?.cid || '1MDusSB9r4r8M2z_pJObLmnlccQH5ZiPL2gwbuwLmIc';
const externalEighteenCID = tokenData.externalAssetDependencies?.[17]?.cid || '-bqtMXT3Ea9diPL35bQVuhH4SL4hL20dWQWEFmphvjk';
const externalNineteenCID = tokenData.externalAssetDependencies?.[18]?.cid || 'tPQomSVEqDnktsMhXTsEARK2oqzcq4b3fa_gUAz1emM';
const externalTwentyCID = tokenData.externalAssetDependencies?.[19]?.cid || 'yxB4Qg0AoXGxrEbBpNkgl3kbS2AWN0qrWoLiUIQmKCA';
const externalTwentyOneCID = tokenData.externalAssetDependencies?.[20]?.cid || 'iREU4Tm7cLs_MzJ03xl9nZXECl4TkWBvnO8MTV7BJN8';
const externalTwentyTwoCID = tokenData.externalAssetDependencies?.[21]?.cid || 'vBxGzs71ycNZPR8Z--PKozA9KLzHcWqUoT85gsTn7rM';
const externalTwentyThreeCID = tokenData.externalAssetDependencies?.[22]?.cid || 'S742CP0IwlXo72wjZKnNxMOle_fbi4URpVqADd8lwsE';
const externalTwentyFourCID = tokenData.externalAssetDependencies?.[23]?.cid || 'Vdqhlq-Wn4UI4r8Qac4TLZvZYIQFpiS0WdlW6dI3xZo';
const externalTwentyFiveCID = tokenData.externalAssetDependencies?.[24]?.cid || 'YsX_BsOgLvbwi4VYUJhBdftiq2LC4DYV3TUOVZPwZ5Y';
const externalTwentySixCID = tokenData.externalAssetDependencies?.[25]?.cid || '9PElU9RC-F6ahDb3miT7Mso_XmGO7JwS44VDswvaURs';


// This actually needs to be set up by Prohibition.
// We don't have one yet....
const preferredArweaveGateway = tokenData.preferredArweaveGateway;

// In the future when the prohibition gateway is available, it will use
// that instead of the public gateway
...
let drumStemsWithPercussion = ["DNB", "Trap'd"]; // Replace with the names of the drum stems that include percussion
let selectedPercussionStem;
if (drumStemsWithPercussion.includes(selectedDrumStem.name)) {
    // If the selected drum stem includes percussion, select the silent stem for percussion
    selectedPercussionStem = stems.percussion.find(stem => stem.name === "None");
} else {
    ...
    let percussionOptions = stems.percussion.filter(stem => stem.name !== "None");
    selectedPercussionStem = percussionOptions[R.random_int(0, percussionOptions.length - 1)];
}
...
function resizeCanvas() {
    ...
    for (let g of ((targetSize = [...(size = [1.5 * A * (canvas.width / canvas.height), 1.3 * A * (canvas.width / canvas.height), 1 * A * (canvas.width / canvas.height)])]), stars))
        (g.x = Math.random() * canvas.width), (g.y = Math.random() * canvas.height);
}
let rotationDirection = [R.random_dec() > 0.5 ? 1 : -1, R.random_dec() > 0.5 ? 1 : -1, R.random_dec() > 0.5 ? 1 : -1],
    previousAvgFreq = 0,
    threshold = 0.01;
function lerp(A, g, B) {
    return (1 - B) * A + B * g;
...
function generateStars() {
    ...
    for (let g = 0; g < 1000; g++) {
        let B = { x: R.random_num(0, canvas.width), y: R.random_num(0, canvas.height), radius: R.random_num(0.2, 2.5), color: R.random_choice(A), opacity: 0 };
        stars.push(B);
    }
}
function drawStars() {
    for (let A of stars)
        (A.x += A.vx),
            ...
            A.timer++,
            A.timer >= A.interval && (A.opacity < 0.01 ? (A.targetOpacity = Math.random()) : (A.targetOpacity = 0), (A.timer = 0)),
...
function drawShape() {
 ...
 const canvasWidth = canvas.width; // Assuming canvas is the variable holding your canvas element
const maxShapeSize = canvasWidth * 0.6; // 20% of canvas width
const minShapeSize = canvasWidth * 0.3; // 5% of canvas width

if (g - previousAvgFreq > threshold) {
    for (let B = 0; B < size.length; B++) {
        let D = g * (B + 1) * R.random_dec() * 0.45; // Adjust the range of size change here
        let newSize = R.random_bool(0.5) ? size[B] + D : size[B] - D;
        newSize = Math.min(Math.max(newSize, minShapeSize), maxShapeSize); // Ensuring the new size is between minSize and maxSize
